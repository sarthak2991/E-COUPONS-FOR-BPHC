...
const Loginvendor = () => {
  ...
  const handleData = (e) => {
    ...
    {/*verifying phone number with backend */}
    axios.post('http://localhost:5000/vendorlogin',{'phoneno':String(phoneno)},{headers:headers}).then((res) => {
  
    if(res.data.status === 'Error'){
      alert('No vendor registered with this mobile number');setPhoneno('')}
    else{alert("Your OTP is " + otpgenerate);
    setOtpgenerated(otpgenerate);
    const vendorname = res.data.data
    //setting vendorname in local storage
